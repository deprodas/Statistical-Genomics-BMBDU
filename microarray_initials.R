...
anno_primary <- AnnotationDbi::select(hugene10sttranscriptcluster.db,
                                      keys = (featureNames(normalized_data)),
                                      columns = c("SYMBOL", "GENENAME"),
                                      keytype = "PROBEID") 

anno_primary <- subset(anno_primary, !is.na(SYMBOL)) 

# Remove multiple mappings (probe IDs) from the assay data  

anno_grouped <- group_by(anno_primary, PROBEID)
...
all(normalized_express_final$PROBEID %in% anno_primary$PROBEID) 
normalized_express_final$PROBEID %in% anno_primary$PROBEID
class(normalized_express_final$PROBEID) 
class(anno_primary$PROBEID) 

all(anno_primary$PROBEID %in% normalized_express_final$PROBEID) 
anno_primary$PROBEID %in% normalized_express_final$PROBEID
class(anno_primary)
class(normalized_express_final)

Joined_df <- dplyr::left_join(anno_primary, normalized_express_final, by= c("PROBEID"= "PROBEID"))
